...
**Estado actual:** preparación multimodal implementada y una campaña ampliada de 96 ejecuciones de RNN, LSTM, GRU y DLinear, más cuatro controles tabulares. La cohorte verificada contiene 105 muestras, 80 de entrenamiento y 25 de validación. Tres ejecuciones neuronales obtienen un MAE menor que el de la predicción cero, pero ninguna media entre tres semillas mejora ese control. La [comparativa ampliada](reports/baselines/expanded-comparison.md) conserva diagnósticos, curvas, recursos y límites. Los ensayos anteriores de [GRU y DLinear](reports/baselines/reference-variants.md) y [RNN y LSTM](reports/baselines/recurrent-comparison.md) permanecen como registros separados de la muestra de 65 observaciones.
...
La preparación y las representaciones son reanudables. Los ensayos breves guardan checkpoints por época y se ha comprobado recuperación exacta de MLP, GRU, DLinear, RNN y LSTM. La [política de recuperación](docs/engineering/checkpoint-recovery.md) de la futura memoria adaptativa sigue pendiente de implementar con esa arquitectura. El [plan de cómputo](docs/engineering/compute-plan.md) contempla la disponibilidad del equipo durante las 24 horas, sin confundirla con rendimiento máximo sostenido.
