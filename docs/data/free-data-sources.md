...
Una futura orden de actualización podrá reutilizar estos campos, resolver los enlaces corrientes en la página oficial cuando cambien y crear una nueva carpeta fechada. Conservará peticiones fallidas y nunca sobrescribirá snapshots anteriores. Ante HTTP 403 no intentará eludir el rechazo. Ante HTTP 429 respetará la restricción y cualquier `Retry-After`. La validación de formato, límites de tamaño, filas, ausencias, fechas y hash deberá preceder a marcar una descarga como válida.

El sondeo ejecutado está en `tmp/fetch_public_probe.py`, como material temporal. No es una implementación científica ni un servicio de datos en producción. La adquisición se apoya en HTTP y API públicas. Un conector MCP futuro no cambiaría las condiciones del proveedor, los límites de cobertura o la necesidad de conservar procedencia y versiones.
